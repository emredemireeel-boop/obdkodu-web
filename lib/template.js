...
function render(templateName, data = {}) {
  data = {
    robotsDirective: data.isNotFound
      ? 'noindex, nofollow'
      : (data.isNoIndex
        ? 'noindex, follow'
...
function processIf(template, data) {
  // Process innermost #if blocks first (those without nested #if inside),
  // then loop until no more blocks remain — this handles arbitrary nesting.
  let prev;
  do {
    prev = template;

    // Innermost if/else blocks (no nested #if inside either branch)
    template = template.replace(
      /\{\{#if\s+(\w+(?:\.\w+)*)\}\}((?:(?!\{\{#if\s)[\s\S])*?)\{\{else\}\}((?:(?!\{\{#if\s)[\s\S])*?)\{\{\/if\}\}/g,
    ...
    // Innermost if-only blocks (no nested #if inside)
    template = template.replace(
      /\{\{#if\s+(\w+(?:\.\w+)*)\}\}((?:(?!\{\{#if\s)[\s\S])*?)\{\{\/if\}\}/g,
      (match, key, content) => {
        const value = getNestedValue(data, key);
